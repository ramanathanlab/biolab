...
class PatricSecondaryStructureClassificationConfig(TaskConfig):
    ...
    # Name of the task
    name: Literal["PatricSecondaryStructureClassification"] = "PatricSecondaryStructureClassification"
    # Dataset source
...
class PatricSecondaryStructureClassification(Task):
    ...
    def evaluate(self, model: LM):
        ...
        token_embs = np.concatenate([output.embedding[:-1, ] for output in model_outputs], axis=0)
        labels = [res_label for seq_label in task_dataset[self.config.target_col] for res_label in seq_label]
