...
def combine_scores_and_aggregate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Combines score lists and computes mean and standard deviation.
    ...
    """
    # TODO: this is still pretty coupled to the parsing logic.
    group_cols = [
        'model_id',
        'display_name',
        'output_dir',
...
def make_pivot_for_metric(df_agg: pd.DataFrame, metric_name: str) -> pd.DataFrame:
    """Create a pivot table for a metric.

    Tasks as rows model display as columns. The cell entries are formatted as
    "mean ± std", and the columns are sorted so that the leftmost column
